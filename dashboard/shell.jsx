...
function AnchorRail() {
  ...
  const items = [
    ...
    ['Q10', '#Q10', 'Ngành hàng được thảo luận nhiều'],
    ...
    <section className="qsection" id={id}>
      ...
      {soaOnly && soaScope && (
        <div style={{
          ...
          <b>Phạm vi dữ liệu:</b> Chỉ tính trên <b>{soaScope.totalRelevant.toLocaleString()}</b> mentions từ <b>{soaScope.groupIds.length} nhóm SOA</b> ({soaScope.groupNames.join(', ')}) — câu hỏi này đặc thù cho Amazon sellers.
        </div>
      )}
      {children}
...
//
// SHOW_INSIGHT: set to true to re-enable the AI Insight / Insight
// sticky-note under every chart. Currently hidden because the Nhận định
// & Khuyến nghị panel covers the same ground.
const SHOW_INSIGHT = false;

function Insight({ qId, children }) {
  if (!SHOW_INSIGHT) return null;
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      <Narrative>
        ...
        Tổng mentions của <b>{ov.soaGroupCount} nhóm SOA</b> chiếm tỉ trọng <b>{ov.soaPct}% SOV</b> trong{' '}
      ...
      <Narrative>
        ...
        {ov.topPersona && <><b>{ov.topPersona.vn}</b> chiếm tỉ trọng áp đảo với <b>{ov.topPersona.count.toLocaleString()}</b> mentions</>}
        {ov.secondPersona && <>, tiếp đó là <b>{ov.secondPersona.vn}</b> với <b>{ov.secondPersona.count.toLocaleString()}</b> mentions.</>}
      </Narrative>
